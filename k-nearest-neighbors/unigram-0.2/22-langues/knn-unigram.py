...
if __name__ == '__main__':
	...
	data = pd.read_csv('22langues.csv', sep=',') # load dataset

	print(data["language"].value_counts()) # print what is in dataset

	x,y = np.array([string.lower() if isinstance(string, str) else string for string in data['Text']]),np.array(data['language'])
	...
	model = pickle.load(pickle_input) # load model

	y_pred = model.predict(x_test) # predict language of test sample
